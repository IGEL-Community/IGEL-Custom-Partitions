...
mv custom/target/vlc_cp_apparmor_reload custom/vlc/config/bin
mv custom/target/igel-vlc-cp-apparmor-reload.service custom/vlc/lib/systemd/system/
mv custom/target/vlc-cp-init-script.sh custom

cd custom

tar cvjf vlc.tar.bz2 vlc vlc-cp-init-script.sh
mv vlc.tar.bz2 ../..
mv target/vlc.inf ../..
mv igel/*.xml ../..
