...
ACTION="custompart-parallels_${1}"
...
# userhome
PARALLELS_CONFIG="/userhome/.config/2X"
...
case "$1" in
init)
  TUXCHECKSCRIPT="${CP}/opt/2X/Client/scripts/tuxclient-schema-check.sh"
  ...
  # basic persistency
  chown -R user:users "${CP}${PARALLELS_CONFIG}"
  ...
  systemctl --no-block start igel-parallels-cp-apparmor-reload.service
