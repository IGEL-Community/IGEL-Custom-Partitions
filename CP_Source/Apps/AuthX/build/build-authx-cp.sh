...
#set -x
#trap read debug
#https://help.authx.com/linux-ssh-xrdp-logon/
...
if ! compgen -G "${GETVERSION_FILE}" > /dev/null; then
  echo "***********"
  echo "Obtain AuthX Authenticator deb file ${GETVERSION_FILE}, save into $HOME/Downloads and re-run this script "
  echo "https://www.authx.com/"
  echo "***********"
  exit 1
fi
