...
APPARMOR="FALSE"
GETVERSION_FILE="$HOME/Downloads/IBScanUltimate_x64_*.tgz"
GET_FILE2="$HOME/Downloads/devicesvc_unix_*.sh"
MISSING_LIBS_OS11=""
MISSING_LIBS_OS12=""
...
if ! compgen -G "${GETVERSION_FILE}" > /dev/null; then
  ...
  echo "https://integratedbiometrics.com/products/fbi-certified-fingerprint-scanners/kojak"
  echo "***********"
  exit 1
fi

if ! compgen -G "${GETV_FILE2}" > /dev/null; then
  echo "***********"
  echo "Obtain latest package, save into $HOME/Downloads and re-run this script "
  echo "https://integratedbiometrics.com/products/fbi-certified-fingerprint-scanners/kojak"
  echo "***********"
